...
SynthDef( \trainer, { |pulse = 0, mix = 0|
	...
	lo = DynKlank.ar(`[ ({|i|if(i==0,{1},{i})}!40).clump(2).flop[0], {|i|1/20}!20 , 1!20 ], in, freqscale: 25,  freqoffset: 100,  decayscale: pulse.linexp(0.0, 1.0, 0.01, 2, 1) ) * 0.4;
	hi = DynKlank.ar(`[ ({|i|if(i==0,{1},{i})}!40).clump(2).flop[1], {|i|1/20}!20 , 1!20 ], in, freqscale: 100, freqoffset: 100,  decayscale: pulse.linexp(0.0, 1.0, 0.01, 2, 1) ) * 0.4;
